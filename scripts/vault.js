...
function createGalleryModal() {
  ...
  info.innerHTML = ''; // Initialized empty
...
function updateModalImage() {
  ...
  const img = modal.querySelector('img');
  const info = modal.querySelector('.gallery-modal-info');
  const item = __currentGalleryList[__currentGalleryIndex] || {};
  ...
  if (info) {
    ...
    const current = __currentGalleryIndex + 1;
    info.textContent = `${category} > ${current}`;
  }
}

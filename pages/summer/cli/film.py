#!/usr/bin/env python3
"""
cli/film.py — Creatures in the Tall Grass CLI
Usage: python cli/film.py <command> [args]
"""
...
def cmd_read(args):
    """Read a scene: cli/film.py read <n|id>"""
    if not args:
        print(red("Usage: cli/film.py read <scene_number_or_id>"))
        return
    scenes = load_manifest()
...
def cmd_characters(args):
    ...
    print(f"\n  {dim('Usage: cli/film.py characters <name>')}\n")
...
def cmd_notes(args):
    ...
    print(f"\n  {dim('Usage: cli/film.py notes <name>')}\n")
...
def cmd_edit(args):
    ...
    if not args:
        print(red("Usage: cli/film.py edit <scene_number_or_id>"))
        return
    scenes = load_manifest()
    scene, n = resolve_scene(args[0], scenes)
    if not scene:
...
def cmd_search(args):
    ...
    if not args:
        print(red("Usage: cli/film.py search <term>"))
        return
...
def cmd_help(args):
    print()
    print(bold(gold("  cli/film.py — Creatures in the Tall Grass CLI\n")))
    commands = [
    ...
    for cmd, desc in commands:
        print(f"  {cyan(f'cli/film.py {cmd}'):<36} {dim(desc)}")
    print()
